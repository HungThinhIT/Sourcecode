...
<ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
	...
	<li class="m-nav__item">
		<a href="{{ route('spa_showListService') }}" class="m-nav__link">
			<span class="m-nav__link-text">
				Danh sách dịch vụ
			</span>
		</a>
					...
					<table class="m-datatable" id="html_table" width="100%">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									{{number_format($value->ServicesPrice,0,",",".")." VNĐ"}}
								</td>
								<td>
									<a href="{{route('spa_showEditService',['id'=>$value->ServicesId])}}" class="btn btn-info m-btn m-btn--icon btn-sm m-btn--icon-only  m-btn--pill" title="Chỉnh sửa">
										<i class="la la-folder"></i>
									</a>
									<button class="btn btn-danger m-btn m-btn--icon btn-sm m-btn--icon-only  m-btn--pill" data-toggle="modal" data-target="#modal_del" data-url="{{route('spa_deleteService',['id'=>$value->ServicesId])}}" data-title="{{$value->ServicesName}}" title="Xoá">
		...
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">
					Xác nhận xoá dịch vụ <b><span id="title-md-del"></span></b>
				</h5>
			</div>
