...
class CustomerMemberController extends Controller
{
    ...
    public function deleteCustomerMember($idCustomerMember){
        if(CustomerBooking::where("CustomerId" ,"=",$idCustomerMember)->exists()){
            return back()->withErrors("Khách hàng thành viên đang ở trong Spa");
        }
        else{
            $idHistoryMember = HistoryMember::where("CustomerMemberId","=",$idCustomerMember)->value("HistoryMemberId");
            HistoryMemberDetail::where("HistoryMemberId","=",$idHistoryMember)->delete();
            HistoryMember::where("CustomerMemberId","=",$idCustomerMember)->delete();
            $customerMember = CustomerMember::find($idCustomerMember)->delete();
            return back()->with("success_message","Xoá thành công");
        }
