...
class CustomerBookingController extends Controller
{
    ...
    public function cancelCustomer(SessionTableHandleController $sessionTable,$idCustomerBooking){
        DB::beginTransaction();
        try {
            $customerBookingStock = CustomerBooking::find($idCustomerBooking);
            if($customerBookingStock == false ){
                $customerBooking = CustomerBooking::find($idCustomerBooking);
                $customerId = $customerBooking->CustomerId;
                $roomId = $customerBooking->RoomId;
                $staffId = $customerBooking->StaffId;
                ...
                //RESET ROOM AND STAFF
                $room = Room::find($roomId)->increment('RoomBlank');
            ...
            $sessionTable->setSessionCustomer();
            $sessionTable->setSessionStaff();
            return back()->with('success_message','Huỷ dịch vụ thành công');
        }catch(\Illuminate\Database\QueryException $e){
            return back()->with('success_message','Huỷ dịch vụ không thành công !'.$e->getMessage());
            DB::rollback();
        }
    }
}
